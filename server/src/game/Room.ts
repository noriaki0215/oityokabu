...
import { Room as RoomType, MAX_PLAYERS, MIN_PLAYERS } from '../../../shared/types';
import { GameState, RoundResult, PlayerRoundResult, PlayerInGame } from '../../../shared/types';
import { ROLE_MULTIPLIERS } from '../../../shared/types';
import { Player, PlayerInGameState } from './Player';
...
export class Room implements RoomType {
  ...
  private playerTurnIndex: number = 0;
  ...
  startGame(): GameState {
    ...
    // ゲーム状態を初期化
    const playersInGame = this.players.map(p => new PlayerInGameState(p));
  ...
  // 次のプレイヤーのターンを設定
  private setNextPlayerTurn(): boolean {
    if (!this.gameState) return false;
  ...
  drawCard(playerId: string): number | null {
    ...
    if (this.gameState.currentTurnPlayerId !== playerId) return null;

    const player = this.gameState.players.find(p => p.id === playerId);
    if (!player) return null;

    // 既に3枚持っている場合は引けない
    if (player.hand.length >= 3) return null;
  ...
  stand(playerId: string): boolean {
    if (!this.gameState) return false;
    if (this.gameState.currentTurnPlayerId !== playerId) return false;
  ...
  calculateRoundResult(): RoundResult {
    ...
    for (const player of this.gameState.players) {
      ...
      if (result === 'win') {
        ...
        const multiplier = dealer.role ? ROLE_MULTIPLIERS[dealer.role] : 1;
        roundResult = -player.bet * multiplier;
      }
  ...
  // 役をチェック
  private checkPlayerRole(player: PlayerInGame, fieldCard: number): void {
    // アラシ: 3枚同じ数字
    if (player.hand.length === 3 && 
  ...
  private compareHands(
    ...
    if (playerPriority > dealerPriority) return 'win';
    if (playerPriority < dealerPriority) return 'lose';
  ...
  nextRound(): GameState {
    ...
    const playersInGame = this.players.map(p => {
      const pig = new PlayerInGameState(p);
      return pig.toJSON();
    });
