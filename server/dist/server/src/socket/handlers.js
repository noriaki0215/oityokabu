...
function setupSocketHandlers(io) {
    io.on('connection', (socket) => {
        ...
        socket.on('create_room', ({ nickname }) => {
            try {
                const room = GameManager_1.gameManager.createRoom(socket.id, nickname);
        ...
        socket.on('join_room', ({ roomCode, nickname }) => {
            try {
                const room = GameManager_1.gameManager.joinRoom(roomCode, socket.id, nickname);
                if (!room) {
        ...
        socket.on('start_game', () => {
            try {
                const room = GameManager_1.gameManager.getPlayerRoom(socket.id);
                if (!room) {
...
function handleLeaveRoom(socket) {
    const roomCode = GameManager_1.gameManager.getPlayerRoomCode(socket.id);
    if (!roomCode)
        return;
    const { room, isEmpty } = GameManager_1.gameManager.leaveRoom(socket.id);
    if (!room)
        return;
